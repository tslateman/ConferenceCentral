...
SPECIAL_QUERY1_REQUEST = endpoints.ResourceContainer(
    message_types.VoidMessage,
    speaker = messages.StringField(1)
)

SPECIAL_QUERY2_REQUEST = endpoints.ResourceContainer(
    message_types.VoidMessage,
...
# - - - Featured Speaker - - - - - - - - - - - - - - - -
    ...
    def _updateFeaturedSpeaker(request):
        """Use memcache to update featured speaker of a conference
        """
        
        curr_speaker = request.get('speaker')
        
        # grab the websafe conference key of the session's parent conf
        websafeConfKey = request.get('websafeKey')
        confKey = ndb.Key(urlsafe=websafeConfKey)

        sessions = Session.query(ancestor=confKey)

        # check conf sessions for same speaker in multiple sessions
        count = 0
        speaker_sessions = []
        for session in sessions:
            if session.speaker == curr_speaker:
                count = count + 1
...
# - - - Sessions - - - - - - - - - - - - - - - - - - - -
    ...
    def _copySessionToForm(self, sesh):
        ...
        for field in sform.all_fields():
            if hasattr(sesh, field.name):
                # convert Date to date string; just copy others
                if field.name.endswith('date'):
                    setattr(sform, field.name, str(getattr(sesh, field.name)))
                elif field.name.endswith('time'):
                    setattr(sform, field.name, str(getattr(sesh, field.name)))
                else:
                    setattr(sform, field.name, getattr(sesh, field.name))
            elif field.name == "websafeKey":
                setattr(sform, field.name, sesh.key.urlsafe())
    ...
    def createSession(self, request):
        ...
        if user_id != confKey.get().organizerUserId:
            raise endpoints.UnauthorizedException(
                'Need conference organizer to create sessions')
        ...
        if not data['sessionName']:
            raise endpoints.UnauthorizedException(
                'Need Session Name to create session')

        if not data['speaker']:
            raise endpoints.UnauthorizedException(
                'Need Session speaker to create session')
    ...
    def getSessionsInWishList(self, request):
        """Get list of sessions that user has wished for."""
        prof = self._getProfileFromUser() # get user Profile
    ...
    @endpoints.method(SPECIAL_QUERY1_REQUEST, SessionForms,
    ...
    @endpoints.method(SPECIAL_QUERY2_REQUEST, SessionForms,
        path = 'specialquery2', http_method='GET', 
        name='specialquery2')
